...
// Define global, 'extern'-ed in ski.h
...
int8_t inkey() {
    ...
    int8_t the_key = KEY_ERROR_CONDITION;
    ...
    while (true) {
        ...
        if (gpio_get(PIN_TURN_BUTTON)) {
            the_key = PIN_TURN_BUTTON;
            is_pressed = true;
        } else if (gpio_get(PIN_PAUSE_BUTTON)) {
            the_key = PIN_PAUSE_BUTTON;
            is_pressed = true;
        } else {
            is_pressed = false;
        }
        ...
        if (is_pressed) {
            if (debounce_count == 0) {
                debounce_count = now;
            } else if (now - debounce_count > DEBOUNCE_TIME_US) {
                debounce_count == 0;
                was_pressed = true;
            }
        } else if (was_pressed) {
